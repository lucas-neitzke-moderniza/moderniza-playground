...
const DataviewResponsive = (callback, currentLayout, deviceSize, optionsResponsive) => {
  ...
  if (deviceSize.width > 0 && deviceSize.width < 576) {
    // console.log('size xs')
    callback(fromDefault(currentLayout, optionsResponsive?.xs))
  } else if (deviceSize.width > 576 && deviceSize.width < 768) {
    // console.log('size sm')
    callback(fromDefault(currentLayout, optionsResponsive?.sm))
  } else if (deviceSize.width > 768 && deviceSize.width < 992) {
    // console.log('size md')
    callback(fromDefault(currentLayout, optionsResponsive?.md))
  } else if (deviceSize.width > 992 && deviceSize.width < 1200) {
    // console.log('size lg')
    callback(fromDefault(currentLayout, optionsResponsive?.lg))
  } else if (deviceSize.width > 1200 && deviceSize.width < 1400) {
    // console.log('size xl')
    callback(fromDefault(currentLayout, optionsResponsive?.xl))
  } else {
    // console.log('size xxl')
    callback(fromDefault(currentLayout, optionsResponsive?.xxl))
  }
}
